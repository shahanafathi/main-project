...
from .models import CustomeUser,medicine,cart,order,wishlist,Category,prescription,company_order,Delivary,order_delivery,product
...
def payment_page(request,am):
    ...
    return render(request, 'user/payment1.html', {'order_item': order_item,'total_price':am})
...
def add_medicine(request):
    ...
    if request.method == 'POST':
        ...
        # print(Image)
        medicine_data = medicine.objects.create( pharmacy_id=data,name=name,Image=Image,price=price, per_price=per_price,Image1=Image1,Image2=Image2,Image3=Image3,strength=strength,description=description,manufacturer=manufacturer,quantity=quantity,category_name=categoryid.category_name,expiry_date=expiry_date)
        medicine_data.save()
...
def delivery_Register(request):
    if request.method == 'POST':
            ...
            data=CustomeUser.objects.create_user(password=password,username=username,usertype='delivery')
            data.save()
            print(data, "dtaaaaaa")
            data1 = Delivary.objects.create(delivary_id=data,name=name,image=image,email=email,licence=License,address=address,phonenumber=phoneno,age=Age)
            data1.save()
            return render(request,'login.html')
    else:
